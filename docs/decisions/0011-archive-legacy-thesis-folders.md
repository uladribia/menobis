...
The thesis-era C/Python folders were removed from the working tree. MENoBiS now
...
|---|---|
| `1. Network analysis/` | `menobis.analysis`, `menobis.data`, Rust graph/stat kernels |
| `2. Model Fitting/` | `menobis.models.fitting`, Rust fitting kernels |
| `3. Model Generation/` | `menobis.models.generation`, Rust sampling providers |
...
The git history remains the archive. New work must extend the modern MENoBiS API,
...
coverage now lives in `tests/test_menobis_legacy_regression.py`: a stable fixture
