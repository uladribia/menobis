"""Shared p-value classification helpers for MENoBiS filtering."""
...
def _significance_masks(
    ...
    if correction == "fdr":
        upper_mask = np.asarray(
            _menobis.benjamini_hochberg(upper.tolist(), upper_alpha)
        )
        lower_mask = np.asarray(
            _menobis.benjamini_hochberg(lower.tolist(), lower_alpha)
        )
