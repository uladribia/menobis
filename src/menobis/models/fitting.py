"""Lagrange multiplier fitting for MENoBiS multi-edge models."""
...
def _solve_zero_truncated_poisson_rate(positive_mean: float) -> float:
    """Solve q/(1-exp(-q)) = positive_mean for ME degree-events.

    This is the thesis positive-support Poisson conditional mean equation used
    by zero-inflated degree-events models.
    """
    if positive_mean < 1.0:
        msg = "positive mean must be at least one"
        raise ValueError(msg)
    if positive_mean == 1.0:
        return 0.0
    lo = 0.0
    hi = max(1.0, positive_mean)
    while hi / (-np.expm1(-hi)) < positive_mean:
        hi *= 2.0
    for _ in range(100):
        mid = 0.5 * (lo + hi)
        value = mid / (-np.expm1(-mid))
...
def _positive_binomial_mean(q: float, layers: int) -> float:
    """Return M q(1+q)^(M-1)/((1+q)^M-1)."""
    if q == 0.0:
        return 1.0
    base = 1.0 + q
    return layers * q * base ** (layers - 1) / (base**layers - 1.0)


def _solve_positive_binomial_rate(positive_mean: float, layers: int) -> float:
    """Solve the B degree-events positive-support binomial equation."""
    if layers < 1:
        msg = "binomial layers must be positive"
        raise ValueError(msg)
    if positive_mean < 1.0:
        msg = "positive mean must be at least one"
        raise ValueError(msg)
    if positive_mean > float(layers):
        msg = "positive mean cannot exceed binomial layers"
        raise ValueError(msg)
    if positive_mean == 1.0:
        return 0.0
    if positive_mean == float(layers):
    ...
    lo = 0.0
    hi = 1.0
    while _positive_binomial_mean(hi, layers) < positive_mean:
        hi *= 2.0
    for _ in range(100):
        mid = 0.5 * (lo + hi)
        if _positive_binomial_mean(mid, layers) < positive_mean:
            lo = mid
        else:
            hi = mid
    return 0.5 * (lo + hi)
...
def fit_strength_cost_poisson_coordinates(
    ...
    *,
    ...
    x_list, y_list, gamma, converged, iters = (
        _menobis.fit_strength_cost_poisson_coordinates(
            s_out.tolist(),
            s_in.tolist(),
            coord_x.tolist(),
            coord_y.tolist(),
...
def fit_strength_cost_binomial_coordinates(
    ...
    *,
    ...
    t0 = time.perf_counter()
    x_list, y_list, gamma, converged, iters = (
        _menobis.fit_strength_cost_binomial_coordinates(
            s_out.tolist(),
            s_in.tolist(),
            coord_x.tolist(),
            coord_y.tolist(),
...
def fit_strength_edges_poisson(
    ...
    *,
    ...
    t0 = time.perf_counter()
    x_list, y_list, lam, converged, iters = _menobis.fit_strength_edges_poisson(
        s_out.tolist(),
        s_in.tolist(),
        target_edges,
        self_loops,
...
def fit_strength_edges_geometric(
    ...
    *,
    ...
    result = _wrap_w_strength_edges_fit(
        _menobis.fit_strength_edges_geometric(
            s_out.tolist(),
            s_in.tolist(),
            float(target_edges),
            self_loops,
...
def fit_strength_edges_negative_binomial(
    ...
    *,
    ...
    t0 = time.perf_counter()
    result = _wrap_w_strength_edges_fit(
        _menobis.fit_strength_edges_negative_binomial(
            s_out.tolist(),
            s_in.tolist(),
            float(target_edges),
            layers,
...
def fit_strength_degree_poisson(
    ...
    *,
    ...
    x_list, y_list, z_list, w_list, converged, iters = (
        _menobis.fit_strength_degree_poisson(
            s_out.tolist(),
            s_in.tolist(),
            k_out.tolist(),
            k_in.tolist(),
...
def fit_strength_poisson(
    ...
    *,
    ...
    Returns:
        StrengthFit with node ids and x, y multipliers.
    """
    s_out = np.asarray(strength_out, dtype=np.float64)
    s_in = np.asarray(strength_in, dtype=np.float64)
    ...
    n = len(s_out)
    t0 = time.perf_counter()
    x_list, y_list, converged, iters = _menobis.fit_strength_poisson(
...
def fit_strength_geometric(
    ...
    *,
    ...
    result = _wrap_w_strength_fit(
        _menobis.fit_strength_geometric(
            s_out.tolist(), s_in.tolist(), self_loops, tolerance, max_iterations
        ),
        node_count=len(s_out),
        self_loops=self_loops,
...
def fit_strength_negative_binomial(
    ...
    *,
    ...
    t0 = time.perf_counter()
    result = _wrap_w_strength_fit(
        _menobis.fit_strength_negative_binomial(
            s_out.tolist(),
            s_in.tolist(),
            layers,
            self_loops,
...
def fit_degree_bernoulli(
    ...
    *,
    ...
    Returns:
        DegreeFit with node ids and x, y multipliers.
    """
    k_out = np.asarray(degree_out, dtype=np.float64)
    k_in = np.asarray(degree_in, dtype=np.float64)
    _validate_balanced_sequences(k_out, k_in, name="degree")
    ...
    t0 = time.perf_counter()
    x_list, y_list, converged, iters = _menobis.fit_degree_bernoulli(
...
def fit_strength_binomial(
    ...
    *,
    ...
    n = len(s_out)
    t0 = time.perf_counter()
    x_list, y_list, converged, iters = _menobis.fit_strength_binomial(
...
def fit_strength_edges_binomial(
    ...
    *,
    ...
    _validate_binomial_feasibility(s_out, s_in, layers, self_loops=self_loops)
    t0 = time.perf_counter()
    x_list, y_list, lam, converged, iters = _menobis.fit_strength_edges_binomial(
        s_out.tolist(),
        s_in.tolist(),
        target_edges,
        layers,
...
def fit_strength_degree_binomial(
    ...
    *,
    ...
    t0 = time.perf_counter()
    x_list, y_list, z_list, w_list, converged, iters = (
        _menobis.fit_strength_degree_binomial(
            s_out.tolist(),
            s_in.tolist(),
            k_out.tolist(),
            k_in.tolist(),
...
def fit_degree_events_poisson(
    ...
    *,
    ...
    q = _solve_zero_truncated_poisson_rate(positive_mean)
    _log_fit_result(
...
def fit_degree_events_binomial(
    ...
    *,
    ...
    positive_mean = t / e if e > 0 else 1.0
    q = _solve_positive_binomial_rate(positive_mean, layers)
    _log_fit_result(
...
def fit_degree_events_geometric(
    ...
    *,
    ...
    x_list, y_list, q, positive_mean, converged, iters = (
        _menobis.fit_degree_events_geometric(
            k_out.tolist(),
            k_in.tolist(),
            int(total_events),
            self_loops,
...
def fit_degree_events_negative_binomial(
    ...
    *,
    ...
    t0 = time.perf_counter()
    x_list, y_list, q, positive_mean, converged, iters = (
        _menobis.fit_degree_events_negative_binomial(
            k_out.tolist(),
            k_in.tolist(),
            int(total_events),
            layers,
