...
When code moves, all call sites update in the same change. When APIs change shape, old signatures are deleted, not preserved.
...
2. Always acquire the relevant context based on code, not docs. Docs and code might diverge (and code should be the source of truth).
3. Keep changes small and targeted.
4. Use TDD:
   - plan code changes, ask question to the user if need be;
   - implement code changes;
   - write tests and check them;
   - refactor after tests pass.
5. Use property-based tests where invariants are more robust than fixed examples.
...
Never perform substantial rewrite work directly on `master`.
...
- Rust-native kernels for MENoBiS graph statistics; do not use external graph libraries such as NetworkX or rustworkx, but be compatible with them when possible.
...
- Write down the thesis equation being implemented in a code comment or
  docstring.
- Add a unit test that verifies the implemented formula against the dense reference test case (PA geographical)
...
- The formulation follows a clear ontology:
  ...
  - All cases contemplate different constraints at three levels. When a constraint reaches saturation level (like total degree=N or N-1), it must be handled specifically and deducted from the problem.
...
- Each grand-canonical family depends on `q_ij = x_i y_j f_ij`, where `f_ij` only applies when cost constraints are considered. Zero-inflated constraints additionally use a binary multiplier `l_ij = w_i z_j` for degree related constraints.
  - Non zero inflated:
    - ME/Poisson: `E[t_ij] = q_ij \in (0,\infty)`.
    - B/Binomial(M): `E[t_ij] = M q_ij / (1+q_ij)` with `q_ij \in (0,\infty)`.
    - W/NegBin(M): `E[t_ij] = M q_ij / (1 - q_ij)` with `q_ij \in (0,1)`. This is a key challenge for solvers.
  - Zero inflated:
...
All new public elements require Markdown documentation. Documents should only be updated after major works have been completed (like branch merges).
...
1. **Generate** a realistic weighted directed network with projected XY
   coordinates using a gravity-like model (present in the synthethic module), use always the DENSE scenario (unsaturated binary constraints and non sparse occupation numbers).
...
- Never use dense `N x N` matrices, if you need to use them, pause and ask the user about it.
