...
def test_no_interop_package_is_shipped() -> None:
    """MENoBiS should not ship graph-library adapter packages."""
    interop_dir = Path(__file__).parents[1] / "src" / "menobis" / "interop"
    assert not interop_dir.exists()


def test_project_has_no_graph_library_runtime_dependency() -> None:
    """MENoBiS runtime dependencies must stay decoupled from graph libraries."""
