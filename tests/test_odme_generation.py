...
from odme.models import fit_fixed_strength_me, sample_multinomial, sample_poisson
...
def test_poisson_positive_weights() -> None:
    x, y = _xy(50, 5000)
    sample = sample_poisson(x, y, seed=42)
...
def test_poisson_reproducible() -> None:
    x, y = _xy(20, 2000)
    a = sample_poisson(x, y, seed=7)
    b = sample_poisson(x, y, seed=7)
...
def test_multinomial_preserves_total() -> None:
    x, y = _xy(50, 5000)
    sample = sample_multinomial(x, y, total_events=5000, seed=42)
    assert sample.total_events == 5000


def test_multinomial_reproducible() -> None:
    x, y = _xy(20, 2000)
    a = sample_multinomial(x, y, total_events=2000, seed=7)
    b = sample_multinomial(x, y, total_events=2000, seed=7)
    np.testing.assert_array_equal(a.weight, b.weight)


def test_no_self_loops() -> None:
    x, y = _xy(10, 1000)
    sample = sample_poisson(x, y, self_loops=False, seed=42)
...
def test_poisson_mean_approaches_expected() -> None:
    ...
    for seed in range(200):
        s = sample_poisson(x, y, seed=seed)
