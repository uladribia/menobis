...
def test_no_self_loops_recovers_strengths() -> None:
    ...
    r = fit_strength_poisson(s_out, s_in, self_loops=False)
...
def test_differs_from_self_loops() -> None:
    s_out = np.array([10, 20, 30], dtype=np.float64)
    s_in = np.array([15, 25, 20], dtype=np.float64)
    with_self = fit_strength_poisson(s_out, s_in, self_loops=True)
    without_self = fit_strength_poisson(s_out, s_in, self_loops=False)
