...
def _degree_events_fit() -> DegreeEventsFit:
...
def test_degree_events_w_filters_partition_edges() -> None:
    ...
    _assert_partitions_edges(filter_degree_events_geometric(_edges(), fit))
    nb_fit = DegreeEventsFit(
        node=fit.node,
        x=fit.x,
        y=fit.y,
        q=fit.q,
        positive_mean=fit.positive_mean,
        self_loops=True,
        converged=True,
        iterations=3,
    ...
    _assert_partitions_edges(filter_degree_events_negative_binomial(_edges(), nb_fit))
