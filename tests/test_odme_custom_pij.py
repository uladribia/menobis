...
from odme.models import sample_custom_pij_multinomial, sample_custom_pij_poisson
...
def test_custom_pij_multinomial_preserves_total_and_support() -> None:
    probabilities = normalize_probabilities(
        np.array([0, 1]), np.array([1, 2]), np.array([0.25, 0.75])
    )
    sample = sample_custom_pij_multinomial(probabilities, total_events=100, seed=42)
...
def test_custom_pij_poisson_is_reproducible() -> None:
    probabilities = normalize_probabilities(
        np.array([0, 1]), np.array([1, 2]), np.array([0.25, 0.75])
    )
    first = sample_custom_pij_poisson(probabilities, total_events=100, seed=42)
    second = sample_custom_pij_poisson(probabilities, total_events=100, seed=42)
