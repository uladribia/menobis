...
// ---------------------------------------------------------------------------
// Phase 7: degree repair (§14–§15, §24)
// ---------------------------------------------------------------------------

/// Internal degree-repair tuning knobs (§15.4).  Safety limits only, not
/// correctness constants; kept out of the public API.
#[derive(Clone, Copy, Debug)]
pub struct DegreeRepairConfig {
    /// Maximum degree-auxiliary steps per reconstruction restart.
    pub max_steps_per_restart: u64,
    /// Maximum reconstruction restarts before giving up.
    pub max_restarts: u32,
    /// Degree-potential strength used by the repair auxiliary step
    /// (§15.2); the same `degree_auxiliary_step` the production trace
    /// uses, with a configurable lambda (§24).
    pub lambda_repair: f64,
}

impl Default for DegreeRepairConfig {
    fn default() -> Self {
        Self {
...
/// Result summary of a successful degree repair.
#[derive(Clone, Copy, Debug)]
pub struct DegreeRepairOutcome {
    /// Total degree-auxiliary steps used across all restarts.
    pub steps: u64,
    /// Reconstruction restarts performed (0 = first attempt).
    pub restarts: u32,
    /// Half-normalized degree distance `D` of the initial exact-E state.
    pub initial_distance: u64,
    /// Best half-normalized degree distance reached.
    pub best_distance: u64,
}

/// Bring an **exact-E** state to the exact residual degree target using
/// the same degree-biased auxiliary step that production sampling uses
/// (§15): repeatedly propose one complete recorded `K_E` transition and
/// accept endpoints with `min(1, exp(−λ·ΔD))` until `D = 0`.
///
/// §15.1: every proposal endpoint preserves exact residual strengths,
/// exact `E`, valid domain, and valid family support — only the degree
/// vector varies.
///
/// Restart policy (§15.4): on step-budget exhaustion the state is
/// discarded and rebuilt from scratch — randomized fixed-strength
/// construction, structural repair, and edge-count repair to the exact
/// residual `E` — then degree repair retries.  Restart 0 consumes the
/// caller-provided exact-E state; `state` must therefore be E-exact on
/// entry.
///
/// # Errors
///
/// - [`DegreeRepairExhausted`](FixedStrengthError::DegreeRepairExhausted)
///   if every restart exhausts its budget without reaching `D = 0`.
///   An inexact-degree state never enters sampling.
#[allow(clippy::too_many_arguments)]
#[allow(dead_code)] // consumed by the Phase 9 fixed-(s,k) orchestrator
pub(crate) fn repair_to_degree_target(
    state: &mut StrengthState,
    ...
    bridge_config: &BridgeConfig,
    degree: &ResidualDegreeTarget,
    config: &DegreeRepairConfig,
) -> Result<DegreeRepairOutcome, FixedStrengthError> {
    let target = &StrengthTarget::new(problem.family);
    let domain = &problem.domain;
    let initial_distance = degree_distance(&state.row_occ_count, &state.col_occ_count, degree);
    ...
    for restart in 0..config.max_restarts {
        if restart > 0 {
            // Rebuild the whole exact-E start: construction + structural
            // repair + edge repair to the exact residual E (§15.4).
            *state = super::fixed_edges::edge_repair_rebuild(problem, rng)?;
            super::fixed_edges::repair_to_edge_target(
                state,
                problem,
                rng,
                edge_target,
                &super::fixed_edges::EdgeRepairConfig::default(),
            )?;
        }
        debug_assert_eq!(
            state.occupied_count(),
            edge_target,
            "degree repair must start from an exact-E state"
        );

        let mut running = degree_distance_raw(&state.row_occ_count, &state.col_occ_count, degree);
        if running < best_distance {
            best_distance = running / 2;
        }
        let mut record: Vec<Cycle4Proposal> = Vec::new();
        let mut steps: u64 = 0;
        loop {
            if running == 0 {
                // §15.3: exact degree match.
                debug_assert_eq!(
                    state.row_occ_count,
    ...
    Err(FixedStrengthError::DegreeRepairExhausted {
        best_degree_distance: best_distance,
...
mod phase7_tests {
    use super::*;
    use crate::generation::microcanonical::occupation_mcmc::domain::PairDomain;
    use crate::generation::microcanonical::occupation_mcmc::fixed_edges::{
        repair_to_edge_target, BridgeConfig,
    };
    use crate::generation::microcanonical::occupation_mcmc::problem::FixedStrengthProblem;
    use crate::model::family::OccupationFamily;
    use rand::rngs::StdRng;
    ...
    /// Sorted positive-occupation pairs `((src, tgt), occ)`.
    type OccupiedState = Vec<((u64, u64), OccNum)>;

    /// Brute-force enumeration of all tiny occupation tables satisfying
    /// exact strengths / domain (independent of production code).
    fn tiny_states(so: &[OccNum], si: &[OccNum], sl: bool, cap: OccNum) -> Vec<OccupiedState> {
        let n = so.len();
        ...
        fn recurse(
            ...
            rem_o: &mut [OccNum],
            rem_i: &mut [OccNum],
            cur: &mut OccupiedState,
            out: &mut Vec<OccupiedState>,
        ) {
            if idx == cells.len() {
                if rem_o.iter().all(|&x| x == 0) && rem_i.iter().all(|&x| x == 0) {
                    out.push(cur.clone());
                }
                return;
            }
            let (s, t) = cells[idx];
            let maxv = rem_o[s as usize].min(rem_i[t as usize]).min(cap);
            for v in 0..=maxv {
                rem_o[s as usize] -= v;
                rem_i[t as usize] -= v;
                if v > 0 {
                ...
                recurse(idx + 1, cells, cap, rem_o, rem_i, cur, out);
                if v > 0 {
                    cur.pop();
                }
                rem_o[s as usize] += v;
    ...
    fn degrees_of(state: &OccupiedState, n: usize) -> (Vec<u32>, Vec<u32>) {
        let mut o = vec![0u32; n];
        let mut i = vec![0u32; n];
        for &((s, t), _) in state {
            o[s as usize] += 1;
            i[t as usize] += 1;
        }
    ...
    fn half_distance(state: &OccupiedState, n: usize, deg: &ResidualDegreeTarget) -> u64 {
        let (o, i) = degrees_of(state, n);
        let mut raw = 0u64;
        for x in 0..n {
            raw += (o[x] as u64).abs_diff(deg.out[x] as u64);
            raw += (i[x] as u64).abs_diff(deg.in_[x] as u64);
        }
        raw / 2
    }
    ...
    fn run_repair(
        ...
        sl: bool,
        start: &OccupiedState,
        deg: &ResidualDegreeTarget,
        seed: u64,
        config: &DegreeRepairConfig,
    ) -> Result<(OccupiedState, DegreeRepairOutcome), FixedStrengthError> {
        let domain = PairDomain::Complete {
            node_count: so.len(),
            self_loops: sl,
        };
        let problem = FixedStrengthProblem::new(family, so, si, domain, vec![])
            .unwrap()
            .into_residual()
            .unwrap();
        let mut rng = StdRng::seed_from_u64(seed);
        let mut state = StrengthState::new(problem.node_count(), start.clone());
        // The enumerated start is already E-exact; make sure anyway.
        repair_to_edge_target(
            &mut state,
            &problem,
            &mut rng,
            deg.edge_count,
            &crate::generation::microcanonical::occupation_mcmc::fixed_edges::EdgeRepairConfig::default(),
        )
        .unwrap();
        let outcome = repair_to_degree_target(
            &mut state,
            &problem,
            &mut rng,
            deg.edge_count,
    ...
    #[test]
    fn repair_reaches_every_feasible_tiny_target() {
        // §41: enumerate exact-strength states, derive feasible degree
        // targets from actual states, start from other exact-E states
        // where possible, and require repair success under normal budgets.
        type Case = (String, OccupationFamily, Vec<OccNum>, Vec<OccNum>, bool);
        ...
        let config = DegreeRepairConfig::default();
        let mut nontrivial = 0usize;
        let mut fibers_repaired = 0usize;
        for (label, family, so, si, sl) in cases {
            let n = so.len();
            let cap = family_capacity_test(family);
            let states = tiny_states(&so, &si, sl, cap);
            assert!(!states.is_empty(), "{label}: no enumerated states");
            // Group by E.
            let mut by_e: std::collections::BTreeMap<usize, Vec<OccupiedState>> =
                std::collections::BTreeMap::new();
            for s in &states {
                by_e.entry(s.len()).or_default().push(s.clone());
            }
            for (e, group) in by_e {
                if group.len() < 2 {
                    continue;
                }
                // Distinct degree targets in this E-group.
                let mut seen = std::collections::HashSet::new();
                let mut targets: Vec<ResidualDegreeTarget> = Vec::new();
                for s in &group {
                    let (o, i) = degrees_of(s, n);
                    if seen.insert((o.clone(), i.clone())) {
                        targets.push(ResidualDegreeTarget {
                            edge_count: e,
                            out: o,
                            in_: i,
                        });
                ...
                for deg in &targets {
                    // Fiber states; pick a start outside the fiber if one
                    // exists (nontrivial repair), otherwise in-fiber.
                    let in_fiber = |s: &OccupiedState| half_distance(s, n, deg) == 0;
                    let start = group
                        .iter()
                        .find(|s| !in_fiber(s))
                        .or_else(|| group.first())
                        .unwrap()
                        .clone();
                    let start_d = half_distance(&start, n, deg);
                    if start_d > 0 {
                        nontrivial += 1;
                    }
                    let (pairs, outcome) = run_repair(
                        family,
                        so.clone(),
                        si.clone(),
                        sl,
                    ...
                    .unwrap_or_else(|err| {
                        panic!("{label} E={e} k={:?}: repair failed: {err}", deg.out)
                    });
                    assert_eq!(
                        half_distance(&pairs, n, deg),
                        0,
                        "{label} E={e}: repair did not reach D=0"
                    );
                    assert_eq!(pairs.len(), e, "{label}: E drifted");
                    assert_eq!(outcome.initial_distance, start_d);
        ...
        assert!(
            nontrivial >= 1,
            "the config list must contain at least one D>0 repair exercise"
        );
        assert!(fibers_repaired >= 3, "expected several repaired fibers");
    }

    fn family_capacity_test(family: OccupationFamily) -> OccNum {
        match family {
    ...
    #[test]
    fn repair_handcrafted_nontrivial_case() {
        // ME N=3 s=[4,3,2]/[2,3,4] sl=true, E=5: state B has degrees
        // k=(1,2,2)/(2,2,1), target C has k=(2,1,2)/(1,2,2).  Repair from
        // B must reach the exact degree fiber of C while keeping strengths
        // and E exact (§15.3).
        let so = vec![4u64, 3, 2];
        let si = vec![2u64, 3, 4];
        let state_b = vec![
        ...
        let problem =
            FixedStrengthProblem::new(OccupationFamily::ME, so.clone(), si.clone(), domain, vec![])
                .unwrap()
                .into_residual()
                .unwrap();
        let mut rng = StdRng::seed_from_u64(3);
        let mut state = StrengthState::new(3, state_b);
        let outcome = repair_to_degree_target(
            &mut state,
            &problem,
            &mut rng,
        ...
        .unwrap();
        assert_eq!(outcome.best_distance, 0);
        assert_eq!(state.row_occ_count, vec![2, 1, 2]);
        assert_eq!(state.col_occ_count, vec![1, 2, 2]);
    ...
    #[test]
    fn repair_infeasible_degree_target_exhausts() {
        // ME N=3 s=[3,3,3]/[3,3,3] sl=true: E=6 is feasible (e.g. the
        // ring state below), but the degree target k_out=(3,2,1),
        // k_in=(1,2,3) is infeasible at E=6: row 2 owns a column with
        // value 3, leaving row 0 only two columns, so k_out[0]=3 is
        // impossible.  The target passes validate_degree_target (necessary
        // checks only) but repair must exhaust with a structured
        // DegreeRepairExhausted (§15.5).
        let so = vec![3u64, 3, 3];
        let si = vec![3u64, 3, 3];
        let deg = ResidualDegreeTarget {
            out: vec![3, 2, 1],
        ...
        let problem = FixedStrengthProblem::new(OccupationFamily::ME, so, si, domain, vec![])
            .unwrap()
            .into_residual()
            .unwrap();
        validate_degree_target(&problem, &deg).unwrap();

        // Start from an exact-E (E=6) ring state with different degrees.
        ...
        let config = DegreeRepairConfig {
            max_steps_per_restart: 3_000,
            max_restarts: 2,
            lambda_repair: 1.0,
        };
        match repair_to_degree_target(
            &mut state,
            &problem,
            &mut rng,
            6,
            ...
            Err(FixedStrengthError::DegreeRepairExhausted {
                ...
                assert!(best_degree_distance >= 1);
                assert_eq!(restarts, 2);
                assert!(total_steps > 0);
                assert_eq!(target_edges, 6);
            }
            other => panic!("expected DegreeRepairExhausted, got {other:?}"),
        }
    }

    #[test]
    fn repair_reproducible_by_seed() {
        // Same seed from the same start must reproduce the repaired
        // endpoint; the handcrafted B->C case has a unique D=0 state, so
        // reproducibility is endpoint-level.
        let so = vec![4u64, 3, 2];
        let si = vec![2u64, 3, 4];
        let state_b = vec![
            ((0, 2), 4),
        ...
        assert_eq!(run(9), run(9), "same seed must reproduce the repair");
        let _ = run(10);
    }
}
